class Publication < ApplicationRecord
  ...
  has_many :shipping_equipments, dependent: :destroy
  has_many :equipments, through: :shipping_equipments
  ...
  scope :by_length_max, -> (value_max) { where("length <= ?", value_max) }
  scope :by_height_min, -> (value_min) { where("height >= ?", value_min) }
  scope :by_height_max, -> (value_max) { where("height <= ?", value_max) }
